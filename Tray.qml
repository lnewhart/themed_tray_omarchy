...
BarWidget {
  ...
  component TrayIcon: Item {
    ...
    // Recolor every tray icon to the bar foreground rather than only
    // freedesktop "-symbolic" ones. Most tray apps (Steam, and anything
    // installed later) ship fixed-color glyphs that clash with the active
    // theme; forcing them through the same tint keeps all tray icons
    // consistent automatically, with no per-app list to maintain. Set to
    // false on a specific TrayIcon instance to opt an app out and keep its
    // native colors.
    property bool forceSymbolic: true
  ...
  component TrayItem: Item {
    ...
    TrayIcon {
      ...
      // Add an app id here (see TrayModel.itemNamed) to opt it out of theme
      // tinting and keep its native colors, e.g.:
      // forceSymbolic: !TrayModel.itemNamed(trayItemRoot.modelData, "some-app")
    }
